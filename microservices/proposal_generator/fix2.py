...
# insert the function before format_indian
target_str = "def format_indian("
idx = content.find(target_str)
if idx != -1:
    content = content[:idx] + capex_code + content[idx:]
    
    # now hook it up in generate_proposal
    target_str_2 = "if 'yearly_savings_chart' in undeclared:"
    idx2 = content.find(target_str_2)
    if idx2 != -1:
        end_idx2 = content.find("doc.render(context)", idx2)
        hook_code = """
