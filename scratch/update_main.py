...
def create_capex_evaluation_sheet(doc, context):
    ...
    master_table.columns[1].width = Inches(0.2)
...
# We need to replace the old create_capex_evaluation_sheet and its imports
pattern = re.compile(r'def create_capex_evaluation_sheet\(doc, context\):.*?return InlineImage\(doc, memfile, width=Inches\(7\.0\)\)', re.DOTALL)
new_content = pattern.sub(new_helpers, content)
