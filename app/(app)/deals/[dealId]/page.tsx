...
export default function DealDetailsPage() {
  ...
  // --- Chart Data Logic ---
  const capacityKw = deal?.kilowatt || 0;
  const unitRate = 8; // Default unit rate

  const monthlyChartData = useMemo(() => {
    if (capacityKw <= 0) return [];
    const generationPerDay = capacityKw * 4;
    const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
    const daysInMonth = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
    const seasonalFactors = [0.95, 0.97, 1.10, 1.13, 1.14, 0.93, 0.75, 0.79, 0.87, 1.02, 1.00, 0.99];

    return months.map((month, index) => {
      const value = generationPerDay * daysInMonth[index] * seasonalFactors[index];
      return {
        name: month,
  ...
  const yearlySavingsData = useMemo(() => {
    if (capacityKw <= 0) return [];
    const initialGenerationPerYear = capacityKw * 4 * 365;
    const years = Array.from({ length: 30 }, (_, i) => i + 1);
    
    let currentGeneration = initialGenerationPerYear;
    let currentRate = unitRate;
    
    return years.map(year => {
      const savings = currentGeneration * currentRate;
      currentGeneration *= 0.996; // 0.4% degradation
          ...
          <div className="flex-grow overflow-y-auto p-6 scrollbar-thin">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              ...
              <div className="lg:col-span-7 space-y-6">
                ...
                {/* Solar Performance Analysis Charts */}
                {capacityKw > 0 && (
                  <Card className="shadow-sm border-border/50">
                    <CardHeader className="pb-3 border-b bg-muted/5 rounded-t-xl">
                      <CardTitle className="text-md">Solar Performance Analysis</CardTitle>
                      <CardDescription>Based on {capacityKw} kW system size</CardDescription>
                    </CardHeader>
                    <CardContent className="p-5 space-y-8">
                      {/* Monthly Generation Chart */}
                      <div className="space-y-4">
                        <h3 className="text-sm font-semibold text-center text-muted-foreground">Monthly Energy Generation</h3>
                          ...
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={monthlyChartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                              ...
                              <YAxis tickLine={false} axisLine={false} tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 12 }} 
                                label={{ value: 'Energy Produced (kWh)', angle: -90, position: 'insideLeft', style: { textAnchor: 'middle', fill: 'hsl(var(--muted-foreground))', fontSize: 12 } }} 
                              />
                              <Tooltip cursor={{ fill: 'hsl(var(--muted)/0.3)' }} contentStyle={{ borderRadius: '8px', border: '1px solid hsl(var(--border))' }} />
                              <Bar dataKey="Energy" fill="#4F81BD" radius={[4, 4, 0, 0]} maxBarSize={50}>
                                <LabelList dataKey="Energy" position="top" style={{ fill: 'hsl(var(--muted-foreground))', fontSize: 10 }} />
                              </Bar>
                            </BarChart>
                          </ResponsiveContainer>
                        </div>
                      ...
                      <div className="space-y-4">
                        <h3 className="text-sm font-semibold text-center text-muted-foreground">Projected Savings for 30 Years</h3>
                        <div className="h-[250px] w-full">
                          <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={yearlySavingsData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                              <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--muted))" />
                              <XAxis dataKey="year" tickLine={false} axisLine={false} tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 12 }} 
                                label={{ value: 'Year', position: 'insideBottom', offset: -5, style: { fill: 'hsl(var(--muted-foreground))', fontSize: 12 } }}
                              />
                              <YAxis tickLine={false} axisLine={false} tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 12 }} tickFormatter={(value) => `₹${(value / 100000).toFixed(1)}L`}
                                label={{ value: 'Projected Savings (₹)', angle: -90, position: 'insideLeft', style: { textAnchor: 'middle', fill: 'hsl(var(--muted-foreground))', fontSize: 12 } }}
                              />
                              <Tooltip 
                              ...
                              />
                              <Line type="monotone" dataKey="Savings" stroke="#F79646" strokeWidth={3} dot={{ r: 4, fill: '#F79646' }} activeDot={{ r: 6 }} />
                            </LineChart>
                          </ResponsiveContainer>
                        </div>
                      </div>
